...
(function () {
  ...
  function U(a, b) {
    ...
    for (; 4 <= a.available(); ) {
      ...
      if (d === y.ECI)
        ...
        d = [];
        f = "";
        for (g = e.readBits([9, 11, 13][b]); 2 <= g; ) (l = e.readBits(11)), (k = Math.floor(l / 45)), (l %= 45), d.push(B[k].charCodeAt(0), B[l].charCodeAt(0)), (f += B[k] + B[l]), (g -= 2);
        1 === g && ((e = e.readBits(6)), d.push(B[e].charCodeAt(0)), (f += B[e]));
        d = { bytes: d, text: f };
        c.text += d.text;
        c.bytes.push(...d.bytes);
        c.chunks.push({ type: t.Alphanumeric, text: d.text });
      } else if (d === y.Byte) (d = T(a, b)), (c.text += d.text), c.bytes.push(...d.bytes), c.chunks.push({ type: t.Byte, bytes: d.bytes, text: d.text });
      else if (d === y.Kanji) {
        f = a;
        d = [];
  ...
  function ka(a) {
          ...
          if (h) {
            ...
            k = c.filter(
              (b) =>
                ...
                (d <= b.bottom.startX && a >= b.bottom.endX && 1.5 > n[2] / (b.bottom.endX - b.bottom.startX) && 0.5 < n[2] / (b.bottom.endX - b.bottom.startX))
            );
          ...
          if (g) {
            ...
            h = e.filter(
              (b) =>
                ...
                (c <= b.bottom.startX && a >= b.bottom.endX && 1.5 > n[2] / (b.bottom.endX - b.bottom.startX) && 0.5 < n[2] / (b.bottom.endX - b.bottom.startX))
            );
    ...
    b = b
      ...
      .map((b) => {
        ...
        if (a.get(Math.round(c), Math.round(d)))
          return (b = [b.top.endX - b.top.startX, b.bottom.endX - b.bottom.startX, b.bottom.y - b.top.y + 1]), (b = u(b) / b.length), { score: O({ x: Math.round(c), y: Math.round(d) }, [1, 1, 3, 1, 1], a), x: c, y: d, size: b };
      })
      .filter((a) => !!a)
    ...
    c = P(a, d, l, q, p);
    b = [];
    c && b.push({ alignmentPattern: { x: c.alignmentPattern.x, y: c.alignmentPattern.y }, bottomLeft: { x: p.x, y: p.y }, dimension: c.dimension, topLeft: { x: q.x, y: q.y }, topRight: { x: l.x, y: l.y } });
    c = H(a, l);
    e = H(a, q);
    f = H(a, p);
    (d = P(a, d, c, e, f)) && b.push({ alignmentPattern: { x: d.alignmentPattern.x, y: d.alignmentPattern.y }, bottomLeft: { x: f.x, y: f.y }, topLeft: { x: e.x, y: e.y }, topRight: { x: c.x, y: c.y }, dimension: d.dimension });
    ...
    b = b
      .map((b) => {
        ...
        if (a.get(Math.floor(c), Math.floor(d)))
          return u([b.top.endX - b.top.startX, b.bottom.endX - b.bottom.startX, b.bottom.y - b.top.y + 1]), (b = O({ x: Math.floor(c), y: Math.floor(d) }, [1, 1, 1], a) + x({ x: c, y: d }, l)), { x: c, y: d, score: b };
      })
      .filter((a) => !!a)
      .sort((a, b) => a.score - b.score);
  ...
  function I(a, b, c, d = {}) {
    ...
    else for (p = 0; p < c; p++) for (m = 0; m < b; m++) (r = 4 * (p * b + m)), q.set(m, p, g.red * a[r] + g.green * a[r + 1] + g.blue * a[r + 2]);
    g = Math.ceil(b / 8);
    p = Math.ceil(c / 8);
    m = g * p;
    if (h) {
    ...
    for (m = 0; m < p; m++)
      for (r = 0; r < g; r++) {
        ...
        24 >= t - v && ((u = v / 2), 0 < m && 0 < r && ((t = (n.get(r, m - 1) + 2 * n.get(r - 1, m) + n.get(r - 1, m - 1)) / 4), v < t && (u = t)));
        n.set(r, m, u);
      }
    ...
    for (b = 0; b < p; b++)
      for (a = 0; a < g; a++) {
        ...
        c = k / 25;
        for (h = 0; 8 > h; h++) for (k = 0; 8 > k; k++) (l = 8 * a + h), (v = 8 * b + k), (t = q.get(l, v)), m.set(l, v, t <= c), f && r.set(l, v, !(t <= c));
      }
