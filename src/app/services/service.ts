...
export class Services {
	...
	storeSession({accessToken}: {accessToken?: string;}): void {
		if (accessToken) {
	...
	public updateMemberFeatures(data: any) {
		Utility.log(data)
		return this.http.post('rest/members/updateMemberFeatures', data, this.headerOptions).pipe(tap (data => {
			
		},
		(err) => {
			Utility.log('updateMemberFeatures() Error...');
			Utility.log(err);
		}));
	}
}
